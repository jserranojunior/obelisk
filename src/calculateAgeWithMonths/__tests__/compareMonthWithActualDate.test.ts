...
describe("compareMonthWithActualDate()", () => {
  ...
  test("ONE_MONTH_LESS (return aways = 'smaller')", () => {    
    ...
    const ONE_MONTH_LESS = new Date(dateForTest.setMonth(dateForTest.getMonth() - 2)).toLocaleDateString("fr-ca");
    expect(compareMonthWithActualDate(ONE_MONTH_LESS, dateToRef)).toBe("smaller");
  });
    test("ONE_MONTH_LARGEr (return aways = 'larger')", () => {    
    const dateForTest = new Date(dateRefenceString)
    const ONE_MONTH_LESS = new Date(dateForTest.setMonth(dateForTest.getMonth() + 2)).toLocaleDateString("fr-ca");
    expect(compareMonthWithActualDate(ONE_MONTH_LESS, dateToRef)).toBe("larger");
  });
   ...
   test("Actual date (return aways = 'equal')", () => {      
     const SAME_DATE = new Date(dateToRef.getFullYear(), dateToRef.getMonth(),  dateToRef.getDay()).toLocaleDateString("fr-ca");
    console.log("Same date",SAME_DATE, dateToRef)
