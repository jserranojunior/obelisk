...
describe("calcularIdadeComMeses()", () => {
  test("calculateAgeWithMonths two months ago DATE_TWO_MONTHS (return aways = '0 anos e 1 meses')", () => {
    const ACTUAL_DATE = new Date()
    const DATE_TWO_MONTHS = new Date(ACTUAL_DATE.setMonth(ACTUAL_DATE.getMonth() - 2)).toLocaleDateString("fr-ca");
    expect(calculateAgeWithMonths(DATE_TWO_MONTHS)).toBe("0 anos e 1 meses");
  });
  test("calculateAgeWithMonths DATE_ON_YEAR_TWO_MONTHS (return aways = '1 anos e 1 meses')", () => {
    const ACTUAL_DATE = new Date()
    ...
    expect(calculateAgeWithMonths(DATE_ON_YEAR_TWO_MONTHS)).toBe("1 anos e 1 meses");
  });
  ...
  test("calculateAgeWithMonths DATE_FORTY (return aways = '76 anos e 4 meses')", () => {
