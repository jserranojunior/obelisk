...
describe("calculateYearsOfLife()", () => {
 ...
 console.log("dateReference", dateRefenceString)
  test("For years (return aways = '4')", () => {    
    const dateForTest = new Date(dateRefenceString)
    const DATE_FOR_YEARS = new Date(dateForTest.setFullYear(dateForTest.getFullYear() - 4)).toLocaleDateString("fr-ca");
    console.log("anos de vida",DATE_FOR_YEARS, dateForTest, dateRefenceString)
  ...
  test("My years old(return aways = '29')", () => {    
    ...
    console.log("My years old", MY_DATE_IN, dateForTest)
    expect(calculateYearsOfLife(MY_DATE_IN, dateToRef)).toBe(29);
  });
    test("Year with +1 month(return aways = '1')", () => {  
    const dateForTest = new Date(dateRefenceString)
    const DATE_TWO_YEARS = new Date(dateForTest.setFullYear(dateForTest.getFullYear() - 2));
    const DATE_TWO_YEARS_WITH_ONE_MONTH = new Date(DATE_TWO_YEARS.setMonth(DATE_TWO_YEARS.getMonth() + 1)).toLocaleDateString("fr-ca");
    expect(calculateYearsOfLife(DATE_TWO_YEARS_WITH_ONE_MONTH, dateToRef)).toBe(1);
  });
      test("Year with -1 month(return aways = '2')", () => {    
    const dateForTest = new Date(dateRefenceString)
    const DATE_TWO_YEARS = new Date(dateForTest.setFullYear(dateForTest.getFullYear() - 2));
    const DATE_TWO_YEARS_WITH_ONE_MONTH = new Date(DATE_TWO_YEARS.setMonth(DATE_TWO_YEARS.getMonth() - 1)).toLocaleDateString("fr-ca");
    expect(calculateYearsOfLife(DATE_TWO_YEARS_WITH_ONE_MONTH,dateToRef)).toBe(2);
  });
});