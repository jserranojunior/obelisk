...
export function calculateAgeWithMonths(value: any, dateJsToCompare: Date) {
...
export function calculateYearsOfLife(birthToCalculate: string, dateJsToCompare: Date) {
  if (birthToCalculate.length === 10) {
    ...
    if (compareMonthWithActualDate(birthToCalculate,dateJsToCompare) == "smaller") {
      yearsOld - 1
    }
    return yearsOld;
  }else{
...
export function compareMonthWithActualDate(birthToCalculate: string, dateJsToCompare: Date){
      const birthdate = new Date(birthToCalculate);
      ...
      if (monthBirth > monthActual) {
        return "larger"
                    ...
                    console.log(dateActual, birthdate, monthBirth, monthActual)
          return "smaller"
        }
        else if(monthBirth == monthActual) {
...
export function calculateMonthOfLife(birthToCalculate: string, dateJsToCompare: Date) {
  if (birthToCalculate.length === 10) {
    const birthdate = new Date(birthToCalculate);
    const dateActual = dateJsToCompare
    const monthActual = dateActual.getMonth()
    ...
    if (compareMonthWithActualDate(birthToCalculate, dateJsToCompare) == "larger") {
      mesesFalta = monthBirth - monthActual;
      monthCont = mesesFalta;
    }else  if (compareMonthWithActualDate(birthToCalculate, dateJsToCompare) == "smaller") {
      mesesFalta = monthActual - monthBirth;
      monthCont = mesesFalta
    } else  if (compareMonthWithActualDate(birthToCalculate, dateJsToCompare) == "equal") {
